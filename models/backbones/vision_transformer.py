...
from .layers import Mlp, PatchEmbed, SwiGLUFFNFused, MemEffAttention, Attention, Block
...
class DinoVisionTransformer(nn.Module):
    def __init__(
        ...
        if dilation_rates is None:
            dilation_rates = [1] * len(residual_block_indexes)
        elif len(dilation_rates) == 1:
            dilation_rates = dilation_rates * len(residual_block_indexes)

        assert len(dilation_rates) == len(residual_block_indexes), \
            f"dilation_rates length ({len(dilation_rates)}) must match residual_block_indexes length ({len(residual_block_indexes)})"

        dilation_map = {}
        for idx, dilation in zip(residual_block_indexes, dilation_rates):
            dilation_map[idx] = dilation
        ...
        blocks_list = [
            block_fn(
                ...
                dilation_rate=dilation_map.get(i, 1),
