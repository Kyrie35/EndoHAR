...
class Trainer:
    def __init__(self, options):
        ...
        print("num_input_frames", self.num_input_frames) # 3
        print("num_pose_frames", self.num_pose_frames) # 2

        assert self.opt.frame_ids[0] == 0, "frame_ids must start with 0"

        self.use_pose_net = not (self.opt.use_stereo and self.opt.frame_ids == [0])
        print("use_stereo", self.opt.use_stereo) # False
        print("use_pose_net", self.use_pose_net) # True
        print("predictive_mask", self.opt.predictive_mask)# False
        print("include_cls_token", self.opt.include_cls_token)# True
        print("learn_intrinsics",self.opt.learn_intrinsics)
        ...
        if self.use_pose_net:
            ...
            if self.opt.pose_model_type == "separate_resnet":
                ...
                self.models["pose"] = decoders.PoseFusDecoder(
            ...
            if self.opt.learn_intrinsics:
                self.models['intrinsics_head'] = decoders.IntrinsicsFusHead(self.models["pose_encoder"].num_ch_enc)
        ...
        print("predictive_mask", self.opt.disable_automasking)
        ...
        if self.num_pose_frames == 2:
            ...
            for f_i in self.opt.frame_ids[1:]:
                ...
                if f_i != "s":
                    ...
                    encoder_features = self.models["pose_encoder"](torch.cat(inputs_all, 1))
                    pose_inputs = [encoder_features]
                    axisangle, translation, _ = self.models["pose"](pose_inputs)

                    if self.opt.learn_intrinsics:
                        cam_K = self.models['intrinsics_head'](
                            encoder_features, self.opt.width, self.opt.height)
