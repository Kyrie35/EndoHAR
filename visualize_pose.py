...
gt_path = os.path.join(os.path.dirname(__file__), "splits", "endovis", "curve", "gt_poses_sequence3.npz")
gt_local_poses = np.load(gt_path, fix_imports=True, encoding='latin1')["data"]

our_path = os.path.join(os.path.dirname(__file__), "splits", "endovis", "curve", "pred_poses_sequence3.npz")
...
figure1, = ax.plot(points_gt[:, 0, 0], points_gt[:, 1, 0], points_gt[:, 2, 0], c='b', linestyle='-', linewidth=1.6)
# figure2, = ax.plot(points_our[:, 0, 0], points_our[:, 1, 0], points_our[:, 2, 0], c='g', label='Prediction', linestyle='-', linewidth=1.6)
figure2, = ax.plot(points_our[:, 0, 0], points_our[:, 1, 0], points_our[:, 2, 0], c='g',linestyle='-', linewidth=1.6)

# plt.legend()
plt.savefig('sq.png',dpi=600)
plt.show()
