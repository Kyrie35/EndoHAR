...
def evaluate(opt):
    ...
    # filenames3 = readlines(
    #     os.path.join(os.path.dirname(__file__), "splits", "endovis",
    #                  "test_files_sequence3.txt"))
    ...
    dataloader2 = DataLoader(dataset2, 1, shuffle=False,
                            num_workers=opt.num_workers, pin_memory=True, drop_last=False)
    # dataset3 = SCAREDRAWDataset(opt.data_path, filenames3, opt.height, opt.width,
    #                             [0, 1], 4, is_train=False)
    # dataloader3 = DataLoader(dataset3, 1, shuffle=False,
    #                          num_workers=opt.num_workers, pin_memory=True, drop_last=False)
    ...
    pose_decoder = decoders.PoseFusDecoder(pose_encoder.num_ch_enc, 1, 2)
    pose_decoder.load_state_dict(torch.load(pose_decoder_path))

    if opt.learn_intrinsics:
        # intrinsics_decoder = decoders.IntrinsicsHead(pose_encoder.num_ch_enc)
        intrinsics_decoder = decoders.IntrinsicsFusHead(pose_encoder.num_ch_enc)
    ...
    # gt_path_3 = os.path.join(os.path.dirname(__file__), "splits", "endovis", "curve", "gt_poses_sequence3.npz")
    # gt_local_poses_3 = np.load(gt_path_3, fix_imports=True, encoding='latin1')["data"]
    # pred_path_3 = os.path.join(os.path.dirname(__file__), "splits", "endovis", "curve", "pred_poses_sequence3.npz")
    # np.savez_compressed(pred_path_3, data=np.array(pred_poses_3))

    # print("Pred poses:", pred_poses_3.shape[0])
    # print("GT poses:", gt_local_poses_3.shape[0])
    ...
    # num_frames_3 = gt_local_poses_3.shape[0]
    ...
    # for i in range(0, num_frames_3 - 1):
    #     local_xyzs = np.array(dump_xyz(pred_poses_3[i:i + track_length - 1]))
    #     gt_local_xyzs = np.array(dump_xyz(gt_local_poses_3[i:i + track_length - 1]))
    #     local_rs = np.array(dump_r(pred_poses_3[i:i + track_length - 1]))
    #     gt_rs = np.array(dump_r(gt_local_poses_3[i:i + track_length - 1]))
    #     ates_3.append(compute_ate(gt_local_xyzs, local_xyzs))
    #     res_3.append(compute_re(local_rs, gt_rs))
    ...
    # cls_3 = st.t.interval(alpha=0.95, df=len(ates_3) - 1, loc=np.mean(ates_3), scale=st.sem(ates_3))
    ...
    # print("\n   sq3 Trajectory error: {:0.4f}, std: {:0.4f}, 95% cls: [{:0.4f}, {:0.4f}]\n".format(np.mean(ates_3), np.std(ates_3), cls_3[0], cls_3[1]))
    # print("\n   sq3 Rotation error: {:0.4f}, std: {:0.4f}\n".format(np.mean(res_3), np.std(res_3)))

    # if opt.learn_intrinsics:#False
